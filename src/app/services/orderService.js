...
import { sellBarcode, returnBarcode } from "./barcodeService";
    ...
    await session.withTransaction(async () => {
      ...
      for (const item of order.items) {
        const result = await Product.updateOne(
          { _id: item.productId, userId, stock: { $gte: item.quantity } },
        ...
        if (item.barcode) {
          await sellBarcode(userId, item.barcode, order._id, session);
        }
      }
